#!/usr/bin/env python
"""Tests A–H on current config defaults.

Current baseline (= "winner combo", pure gradient):
  edge_specific_G=True, per_neuron_amplitudes=False, learn_lambda_u=False,
  learn_W=False, dynamics_cv_every=0, num_epochs=20, lr=0.001,
  synaptic_lr_multiplier=5, rollout_weight=0.1, rollout_steps=5

Tests:
  A  Baseline (current defaults) vs dynamics-CV every 5
  B  Pure gradient = baseline (already cv_every=0)
  C  Old structural defaults (scalar G, per-neuron a, learn_λ) ± CV
  D  dynamics_cv_every sweep {1, 5, 10, 20}
  E  dynamics_cv_blend sweep {0.3, 0.5, 0.7} at cv_every=5
  F  synaptic_lr_multiplier {10, 20}
  G  learning_rate 0.003
  H  rollout_weight × rollout_steps grid

Run:
  nohup .venv/bin/python -u scripts/sweep_AH.py \
        > output_plots/stage2/sweep_AH/sweep.log 2>&1 &
"""
...
BASE_DIR = "output_plots/stage2/sweep_AH"

# Common: use current defaults for epochs/eval, disable slow outputs
COMMON = dict(
...
RUNS = [
    # ═══ A/B: Baseline vs CV ═══════════════════════════════════════
    # A0: Baseline = current defaults (pure gradient, winner combo)
    ("00_A_baseline",       {}),
    # A1: Winner combo + dynamics-CV every 5
    ("01_A_cv5",            dict(dynamics_cv_every=5)),

    # ═══ C: Old structural defaults ════════════════════════════════
    # C0: Old defaults, pure gradient
    ("02_C_old_nocv",       dict(edge_specific_G=False,
                                 per_neuron_amplitudes=True,
                                 learn_lambda_u=True)),
    # C1: Old defaults + CV every 5
    ("03_C_old_cv5",        dict(edge_specific_G=False,
                                 per_neuron_amplitudes=True,
                                 learn_lambda_u=True,
                                 dynamics_cv_every=5)),

    # ═══ D: dynamics_cv_every sweep ════════════════════════════════
    ("04_D_cv1",            dict(dynamics_cv_every=1)),
    ("05_D_cv10",           dict(dynamics_cv_every=10)),
    ("06_D_cv20",           dict(dynamics_cv_every=20)),
    # (cv_every=0 is 00_A_baseline; cv_every=5 is 01_A_cv5)

    # ═══ E: dynamics_cv_blend at cv_every=5 ════════════════════════
    ("07_E_blend03",        dict(dynamics_cv_every=5, dynamics_cv_blend=0.3)),
    ("08_E_blend05",        dict(dynamics_cv_every=5, dynamics_cv_blend=0.5)),
    ("09_E_blend07",        dict(dynamics_cv_every=5, dynamics_cv_blend=0.7)),
    # (blend=1.0 at cv_every=5 is 01_A_cv5)

    # ═══ F: synaptic LR multiplier ════════════════════════════════
    ("10_F_synlr10",        dict(synaptic_lr_multiplier=10.0)),
    ("11_F_synlr20",        dict(synaptic_lr_multiplier=20.0)),

    # ═══ G: global learning rate ══════════════════════════════════
    ("12_G_lr3e3",          dict(learning_rate=0.003)),

    # ═══ H: rollout weight × steps ════════════════════════════════
    # Current: rollout_weight=0.1, rollout_steps=5 (= baseline)
    ("13_H_roll03_s5",      dict(rollout_weight=0.3)),
    ("14_H_roll10_s5",      dict(rollout_weight=1.0)),
    ("15_H_roll01_s10",     dict(rollout_steps=10)),
    ("16_H_roll03_s10",     dict(rollout_weight=0.3, rollout_steps=10)),
    ("17_H_roll10_s10",     dict(rollout_weight=1.0, rollout_steps=10)),
]
