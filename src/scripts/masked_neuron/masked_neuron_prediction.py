#!/usr/bin/env python3
"""Masked neuron next-frame prediction from lagged population activity.

Models: ridge-linear (RidgeCV), MLP (2-hidden-layer).
Causal modes: inclusive (other neurons at t+1), strict (only up to t).
Evaluation: k-fold contiguous temporal CV.

Usage:
    python -m scripts.masked_neuron.masked_neuron_prediction --neuron AVAR --causal inclusive --device cuda
    python -m scripts.masked_neuron.masked_neuron_prediction --neuron ALL
"""
...
def _build_features(
    ...
    X = np.empty((T_out, N_feat * n_lags), dtype=np.float32)

    if causal_mode == "inclusive":
        for lag in range(n_lags):
            start = n_lags - lag
            X[:, lag * N_feat:(lag + 1) * N_feat] = u[start:start + T_out][:, feature_cols]
...
def _plot_single_neuron(out_dir, neuron_name, y_true, preds, r2s, causal_mode, dt):
    import matplotlib
    matplotlib.use("Agg")
    ...
    model_colors = {"ridge-linear": "#3498db", "mlp": "#e74c3c"}
    ...
    for ax, m in zip(axes, models):
        ...
        ax.plot(t_sec, preds[m], lw=0.8, label=f"{m}  R²={r2s[m]:.3f}",
                alpha=0.85, color=model_colors.get(m, "grey"))
...
def _plot_summary(out_dir, results, causal_mode, n_folds):
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    if "ridge-linear" in models and "mlp" in models:
        rl = np.array([r["r2"]["ridge-linear"] for r in results])
        rm = np.array([r["r2"]["mlp"] for r in results])
        ...
        ax.set_ylabel("R²  mlp")
        ax.set_title(f"Ridge-linear vs MLP  ({tag})")
        ax.set_aspect("equal", adjustable="datalim")
    ...
    mlp_key = "mlp" if "mlp" in models else None
...
def _save_results(out_dir, all_results, all_predictions, args):
    pred_dir = out_dir / "predictions"
    pred_dir.mkdir(parents=True, exist_ok=True)
    for key, data in all_predictions.items():
        np.savez_compressed(pred_dir / f"{key}.npz", **data)
    ...
    for r in all_results:
        ...
        for model, r2_val in r["r2"].items():
            row[f"r2_{model.replace('-', '_')}"] = r2_val
        rows.append(row)
    ...
    print(f"  Predictions saved: {pred_dir}/ ({len(all_predictions)} files)")


def main():
    pa = argparse.ArgumentParser(
        description="Masked neuron next-frame prediction from lagged population")
    ...
    print(f"Models: ridge-linear" + ("" if args.skip_mlp else " + mlp"))
    ...
    for ni, neuron_name in enumerate(target_neurons):
        ...
        for wi, w in enumerate(worms):
            ...
            if not args.skip_mlp:
                t0 = time.time()
                pred_rm, r2_rm = _run_mlp(X, y, args.n_folds, args)
                dt_rm = time.time() - t0
                preds["mlp"] = pred_rm
                r2s["mlp"] = r2_rm
                print(f"    mlp           R²={r2_rm:.4f}  ({dt_rm:.1f}s)")
            ...
            pred_key = f"{neuron_name}_{w['name']}"
            pred_data = {"y_true": y, "dt": np.float32(dt)}
            for m, p in preds.items():
                pred_data[f"pred_{m.replace('-', '_')}"] = p
    ...
    _save_results(out_base, all_results, all_predictions, args)
