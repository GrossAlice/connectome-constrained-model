#!/usr/bin/env python
"""
Stage2 ODE — test per-neuron L2 and linear reparametrisation.

Builds on the S2_equiv_lag5 baseline (LOO=0.3996) by enabling two features
designed to close the gap to U3_full_lag5 Ridge (LOO=0.4802):

Conditions:
  C0_baseline       — exact S2_equiv_lag5 replica (sanity check)
  C1_perneuron      — per_neuron_l2=True  (ARD-style learnable alpha_i)
  C2_linear         — linear_reparametrization=True  (skip sigmoid/softplus)
  C3_both           — per_neuron_l2 + linear_reparametrization (both)
"""
...
SAVE_ROOT = SRC / "output_plots" / "stage2" / "perneuron_linear"
...
CONDITIONS = {
    "C0_baseline":    dict(),
    "C1_perneuron":   dict(per_neuron_l2=True),
    "C2_linear":      dict(linear_reparametrization=True),
    "C3_both":        dict(per_neuron_l2=True, linear_reparametrization=True),
}
...
# ── Final summary ────────────────────────────────────────────────
print(f"\n{'='*60}")
print("FINAL SUMMARY  (per-neuron L2 / linear reparametrisation)")
print(f"{'='*60}")
print(f"{'Condition':<20} {'1step':>8} {'LOO':>8} {'med_LOO':>8} {'time':>6}")
print("-" * 60)
for name, s in results_all.items():
    print(f"{name:<20} {s.get('cv_onestep_r2_mean',0):.4f}   "
